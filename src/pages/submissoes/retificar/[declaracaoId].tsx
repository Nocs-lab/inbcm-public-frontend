...
const schema = z.object({
  ...
  .superRefine(({ museologico, bibliografico, arquivistico }, ctx) => {
    if (museologico.length === 0 && bibliografico.length === 0 && arquivistico.length === 0) {
      ctx.addIssue({
...
const { validate_museologico, validate_arquivistico, validate_bibliografico } = new ComlinkWorker<typeof import('../worker')>(new URL('../worker', import.meta.url), { type: "module" })
...
export default function RetificarDeclaracao() {
  ...
  const { data: declaracao } = useSuspenseQuery<{ museu_id: { _id: string, nome: string }, anoDeclaracao: string }>({ queryKey: ["declaracao", id], queryFn: async () => {
  ...
  const totalFiles = [museologico, bibliografico, arquivistico].filter((file) => file?.length > 0).length
    ...
    <DefaultLayout>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-5">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 mt-3">
          <Input label="Museológico" type="file" error={errors.museologico as FieldError} {...register("museologico")} accept=".xlsx" />
          <Input label="Bibliográfico" type="file" error={errors.bibliografico as FieldError} {...register("bibliografico")} accept=".xlsx" />
          <Input label="Arquivístico" type="file" error={errors.arquivistico as FieldError} {...register("arquivistico")} accept=".xlsx" />
        </div>
        <button type="submit" className={clsx("br-button primary mt-5", isSubmitting || isValidating && "loading")} disabled={totalFiles < 1}>Enviar</button>
      </form>
    </DefaultLayout>
  )
}
