...
export default async function request(path: string, init?: RequestInit & { data?: { [key: string]: string | number | boolean } }): Promise<Response> {
  const headers = { "Accept": "application/x-msgpack", ...init?.headers } as { [key: string]: string }
